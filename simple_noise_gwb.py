...
# Fourier Basis for red noise like GWB

def FourierBasisGPGWB(spectrum, components=20,
    ...
    BaseClass = gp_signals.BasisGP(spectrum, basis, selection=selection, name='rn_gwb')

    class FourierBasisGPGWB(BaseClass):
        signal_type = 'basis'
        signal_name = 'rn_gwb'

    return FourierBasisGPGWB
...
# gwb noise parameters
log10_Agwb = parameter.Uniform(-20, -11)
gamma_gwb = parameter.Constant(4.33)
...
# gwb noise signal
pl_gwb = utils.powerlaw(log10_A=log10_Agwb, gamma=gamma_gwb)
rn_gwb = FourierBasisGPGWB(pl_gwb, components=30)
...
# full model
s = ef + eq + rn + rn_gwb + dmgp + tm + dmys
